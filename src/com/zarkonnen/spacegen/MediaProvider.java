...
public class MediaProvider {
	...
	public BufferedImage scale(BufferedImage src, double scale) {
		...
		final BufferedImage dst = config.createCompatibleImage(sWidth, sHeight, Transparency.BITMASK);
	...
	/**
	 * Loads an image from in-jar resource, relative to
	 * com/metalbeetle/defaultgame/images.
	 * 
	 * @param The image name. Assuming extension of jpg unless png extension is
	 *            supplied.
	...
	public synchronized BufferedImage readImage(String name, int transparency) {
		...
		try {
			BufferedImage fixedImg = config.createCompatibleImage(img.getWidth(), img.getHeight(), transparency);
	...
	/**
	 * Plays a sound loaded from in-jar resource, relative to
	 * com/zarkonnen/play/SEGame/sounds.
	 * 
	...
	public BufferedImage border(BufferedImage src, Color borderC) {
		...
		for (int y = 0; y < h + 2; y++) {
			for (int x = 0; x < w + 2; x++) {
				if (y > 0 && y < h + 1 && x > 0 && x < h + 1 && ar.getSample(x - 1, y - 1, 0) > 0) {
					continue;
				}
				boolean border = false;
				lp: for (int dy = -2; dy < 1; dy++) {
					for (int dx = -2; dx < 1; dx++) {
						if (y + dy < 0 || y + dy >= h || x + dx < 0 || x + dx >= w) {
							continue;
						}
						if (ar.getSample(x + dx, y + dy, 0) > 0) {
							border = true;
							break lp;
						}
					}
...
//TODO Debugging Line
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				c = new Color((c.getRed() * na + (r * c.getRed()) / 256) / 256,
						(c.getGreen() * na + (g * c.getGreen()) / 256) / 256,
						(c.getBlue() * na + (b * c.getBlue()) / 256) / 256, ar.getSample(x, y, 0));
