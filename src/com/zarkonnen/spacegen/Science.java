...
//TODO make it so they can make their stats better
			if (actor.getTechLevel() > 5) {

				int dice = actor.getTechLevel() + actor.getIntelligence();
				int stat = sg.d(5);
				int statval = 0;
				if (stat == 1)
					statval = 6 + actor.getStrength();
				else if (stat == 2)
					statval = 6 + actor.getDexterity();
				else if (stat == 3)
					statval = 6 + actor.getConstitution();
				else if (stat == 4)
					statval = 6 + actor.getCharisma();
				else
					statval = 6 + actor.getIntelligence();

				int checkDif = statval * 10;
				int roll = sg.d(dice, actor.getTechLevel()) + actor.getScience();
				sg.l("The $name attempting to upgrade their stats of value :" + stat + " current score is: "
						+ (statval - 6) + "\nThey difficulty is: " + checkDif + " They rolled " + roll, actor);
				if (roll >= checkDif) {
					if (stat == 1)
						actor.Strength++;
					else if (stat == 2)
